...
function MainScreen({ scoreboards, onScoreboardPress, currentUser, onLogout, onCreateScoreboard }: {
  ...
  const getPendingRequestsCount = (scoreboard: Scoreboard): number => {
    if (scoreboard.status === 'pending') return 0;
...
function ScoreboardScreen({ scoreboard, currentUser, onBack, onScoreboardUpdate, onScoreboardDelete }: {
  ...
  const formatTimestamp = (timestamp: any): string => {
    ...
    if (isNaN(then.getTime())) {
      return 'created at: recently';
    }

    // Format as: created at: YYYY/MM/DD HH:MM:SS
    ...
    const seconds = String(then.getSeconds()).padStart(2, '0');

    return `created at: ${year}/${month}/${day} ${hours}:${minutes}:${seconds}`;
  };
  ...
  useEffect(() => {
    const unsubscribe = firebaseService.subscribeToScoreboard(scoreboard.id, (updatedScoreboard) => {
      try {
        if (updatedScoreboard) {
          // Check if the scoreboard still has valid data
          if (!updatedScoreboard.players || updatedScoreboard.players.length === 0) {
            // Scoreboard has no players, it's being deleted
            Alert.alert(
              'Scoreboard Closed',
              'The other player has left the competition. The scoreboard has been deleted.',
              [{ text: 'OK', onPress: () => {
          ...
          // Check if current user is still in the scoreboard
          const userStillInScoreboard = updatedScoreboard.players.some(
            player => player.name === currentUser.name
          );

          if (!userStillInScoreboard) {
            // Current user was removed
            Alert.alert(
              'Removed from Scoreboard',
              'You have been removed from this scoreboard.',
              [{ text: 'OK', onPress: () => {
                onScoreboardDelete(scoreboard.id);
                onBack();
              }}]
    ...
    return unsubscribe;
  }, [scoreboard.id, onScoreboardUpdate, onScoreboardDelete, onBack, currentUser.name]);

  const addPoint = async (playerId: string, playerName: string) => {
    try {
      // Check if scoreboard still exists and has players
  ...
  const netOutScores = async () => {
    ...
    if (players.length !== 2) {
      Alert.alert('Error', 'Net out requires exactly 2 players.');
      return;
    }
  ...
  if (currentScoreboard.status === 'pending') {
    ...
    const isCreator = currentScoreboard.createdBy === currentUser.id;
    ...
    return (
      <View style={containerStyle}>
        <View style={styles.header}>
          ...
          <Text style={[styles.subtitle, textStyle]}>
            {isCreator ? 'Invitation Sent' : 'Pending Challenge'}
          </Text>
        </View>
  ...
  // Safety check: if no players, show loading or error state
    ...
    return (
      <View style={containerStyle}>
        <View style={styles.header}>
          ...
          <Text style={[styles.title, textStyle]}>Loading...</Text>
        </View>
      </View>
    );
  }
            ...
            >
              <View style={styles.pendingChangeCard}>
                <View style={styles.pendingChangeHeader}>
                  ...
                  <Text style={styles.timestampText}>
                    {formatTimestamp(currentScoreboard.pendingReset.timestamp)}
                  </Text>
                </View>
                <Text style={styles.waitingText}>Waiting for confirmation... (Swipe left to cancel)</Text>
              </View>
            ...
            const repaymentCardContent = (
              <View style={styles.pendingChangeCard}>
                <View style={styles.pendingChangeHeader}>
                  ...
                  <Text style={styles.timestampText}>
                    {formatTimestamp(repayment.timestamp)}
                  </Text>
                </View>
